...
uniform float time;
        ...
        void main()
        {
            ...
            vec2 uv = vUv * 20.;
            float pct = smoothstep(0.4, 1.0, abs(sin(uv.x)));
  
  //vec3 c = mix(vec3(pct), vec3(.0, 0.0, 0.3), step(abs(sin(time)), 0.5));
  vec3 c = vec3(0.,0., abs(sin(time) - 0.2)) * pct;
