...
varying vec3 vColor;
   void main() {
       ...
       float displacement  =  pnoise3(.4 * position + vec3( 0, uTime, 0 ), vec3( 100.0 ) ) * 1. * .7;

       float animation = uAnimation;
       vec3 pos = position;

       pos.x += snoise3(position.xyz * 0.02 + 50.0 + uTime) * (200.0 + extras.y * 800.0) * animation;
       pos.y += snoise3(position.xyz * 0.01 + 2.0 + uTime) * (200.0 + fract(extras.y * 32.0) * 800.0) * animation;
       pos.z += snoise3(position.xyz * 0.03 + 100.0 + uTime) * (200.0 + fract(extras.z * 32.0) * 800.0) *  animation;
       ...
       gl_PointSize = 1.0;
       //gl_PointSize = abs(sin(position.x + uTime));
      }
